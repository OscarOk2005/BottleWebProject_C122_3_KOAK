...
def createhistory(methodname, data):
    ...
    if(methodname == "Prima"):
        ...
        history["Prima"].append({"date":string, "original":data[0], "edge_list":data[1], "mst":data[2]})
    elif(methodname == "Kraskala"):
        # Если алгоритм Краскала, то сохраняем текущую дату и время, список ребер и мст
        history["Kraskala"].append({"date":string, "original":data[0], "edge_list":data[1], "mst":data[2]})
